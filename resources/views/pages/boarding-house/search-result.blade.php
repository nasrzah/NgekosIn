...
@section('content')
    ...
    <div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-[18px]">
        <div class="flex flex-col gap-[6px]">
            <h1 class="font-bold text-[32px] leading-[48px]">Search Result {{ $keyword }}</h1>
    ...
    <section id="Result" class=" relative flex flex-col gap-4 px-5 mt-5 mb-9">
        ...
        @foreach ($results as $result)
            <a href="{{ route('kos.show', $result->slug) }}" class="card">
                <div
                    ...
                    <div class="flex flex-col gap-3 w-full">
                        ...
                        <div class="flex items-center gap-[6px]">
                            ...
                            <p class="text-sm text-ngekos-grey">{{ $result->capacity }} People</p>
                        </div>
                        <hr class="border-[#F1F2F6]">
                        <p class="font-semibold text-lg text-ngekos-orange">
                            Rp.{{ number_format($result->price, 0, ',', '.') }}<span
